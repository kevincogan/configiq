...
export function modelHasTestedConfiguration(modelId: string, testedModels: string[]): boolean {
...
export function modelParameterBillions(modelId: string): number | null {
  const override = MODEL_PARAMETER_OVERRIDES.find(([pattern]) => pattern.test(modelId))
